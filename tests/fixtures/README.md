...
`tagging-golden.json`은 프롬프트 평가용으로 새로 작성한 사용자 요청 12개와
각 요청에서 반드시 찾아야 할 stage 정답입니다. `npm run eval:tagging`이 PR #34
이전 프롬프트와 현재 프롬프트를 같은 모델로 실행하고 단계별 precision/recall과
사례 단위 exact match를 비교합니다. 원본 세션에서 가져온 문장은 없습니다.
