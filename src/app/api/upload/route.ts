...
export async function POST(request: Request): Promise<Response> {
  ...
  if (!form || !(file instanceof File)) {
    return Response.json(
      { error: "file field required (multipart/form-data)" } satisfies ApiError,
      { status: 400 },
    );
  }
  if (file.size === 0 || file.size > MAX_BYTES) {
    return Response.json(
      { error: `file size must be 1B~${MAX_BYTES}B` } satisfies ApiError,
      { status: 413 },
    );
  }
  ...
  if (!detected) {
    ...
    return Response.json(
      {
        error:
          "unsupported log format (Claude Code / Codex JSONL only for now)",
      } satisfies ApiError,
      { status: 422 },
    );
  }
  ...
  if (upErr) {
    return Response.json(
      { error: `storage upload failed: ${upErr.message}` } satisfies ApiError,
      { status: 500 },
    );
  }

  const projectName =
    (form.get("projectName") as string | null)?.slice(0, 200) ||
    file.name ||
    "untitled";
  ...
  const { data: project, error: pErr } = await db
    .from("projects")
    .insert({ name: projectName, repo_url: repoUrl, owner_id: auth.userId })
    .select("id")
    .single();
  if (pErr || !project) {
    return Response.json(
      { error: `project insert failed: ${pErr?.message}` } satisfies ApiError,
      { status: 500 },
    );
  }
  ...
  if (sErr || !session) {
    return Response.json(
      { error: `session insert failed: ${sErr?.message}` } satisfies ApiError,
      { status: 500 },
    );
  }
  ...
  if (jErr || !job) {
    return Response.json(
      { error: `job insert failed: ${jErr?.message}` } satisfies ApiError,
      { status: 500 },
    );
  }
