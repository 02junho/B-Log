...
> 마지막 갱신: 2026-09-16 (D-4). 단계가 끝날 때마다 이 문서를 갱신한다.
...
**9/16 분석 중단·재개 — 역할 B, `codex/session-resume`**

- 대시보드의 업로드·분석 중·분석 완료 기록에 `분석 이어가기`를 연결했다.
- `/sessions/[id]/progress`가 인증과 소유권을 확인하고 최신 parse/tag 잡을 찾아 서버의 `next_idx`부터 이어서 실행한다.
- 분석이 끝난 세션은 검수용 초안을 만들고 검수 화면으로 연결한다. 실패가 확정된 세션은 잘못된 자동 재실행 대신 새 분석을 안내한다.
- 새 재개 URL도 로그인 후 안전한 복귀 대상으로 허용했다. 실제 배포 계정 E2E는 남아 있다.
