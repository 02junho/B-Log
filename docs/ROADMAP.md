...
> 마지막 갱신: 2026-09-06 (D-14). 단계가 끝날 때마다 이 문서를 갱신한다.
...
## Step 4. LLM 모델 · 프롬프트 v1 결정 ⏳ (9/7)

**목표** 4단계 태깅(문제 정의 → AI 지시 → 근거 탐색·의사결정 → 실패·복구) 품질과 비용을 실측해 메인 모델을 정한다.

**후보와 가격 (2026-09 기준, $/1M 토큰)**

| 모델 | 입력 | 출력 | 비고 |
| --- | --- | --- | --- |
| Upstage Solar Pro 4 | 0.30 | 1.20 | 9/10까지 90% 할인, 524K 컨텍스트, OpenAI 호환 |
| Claude Sonnet 5 | 2.00 | 10.00 | 1M 컨텍스트 |

Opus는 이 작업에 과해서 제외. Haiku 4.5는 예비.

**할 일**
- Vercel AI SDK(`ai` + `@ai-sdk/anthropic` + `@ai-sdk/openai-compatible`)로 provider 추상화. 구조화 출력은 zod.
- Step 3 결과에서 청크 10~20개를 뽑아 같은 프롬프트로 두 모델에 태깅 → 태그 일치율, 원문 인용 정확도, 응답 시간, 청크당 비용 비교.
- 임베딩 공급자는 별도 결정 (Anthropic은 임베딩 없음 → Upstage `solar-embedding-1-large` 또는 OpenAI). MVP에서 임베딩은 매칭 3단계(대화록 등급) 전용.

**완료 조건** 메인 모델, 프롬프트 v1, 세션당 비용 추정치.

**필요한 것** `ANTHROPIC_API_KEY`, `UPSTAGE_API_KEY` 발급 후 `.env.local`에 입력 (본인).
