...
> 마지막 갱신: 2026-09-13 (D-7). 단계가 끝날 때마다 이 문서를 갱신한다.
...
**9/13 인증·소유자 보호 — 역할 B 후속 브랜치 `codex/github-auth-ownership` (main 미반영)**
...
- 커밋 `c652d2a`를 `codex/github-auth-ownership`에 푸시했다. 검수 보강 PR #18이 아직 열려 있어 이번 인증 변경은 해당 브랜치를 기준으로 한 후속 PR로 진행한다.
- 전체 테스트 84개 중 82개 통과(선택 실로그 2개 제외), 린트·타입 검사·프로덕션 빌드 통과.

**9/13 마스킹 2단계 완성 — P1 `feat/p1-masking-llm` (main 미반영)**

- 1차 정규식 보강: **Windows 홈 경로**(`C:\Users\<이름>`)를 새로 잡는다. 기존 규칙이 POSIX 경로만
  봐서, Windows에서 개발하는 이 팀의 로그가 그대로 통과하고 있었다. 실로그 1개에서 16건 발화 확인.
  그 외 비밀키 블록·접속 문자열·웹훅 URL·주민번호·카드번호(Luhn 검증)·IPv4 규칙 추가.
- 2차 LLM 검출(`masking/detect.ts`): 모델은 **탐지만** 하고 치환은 코드가 결정론적으로 한다.
  모델에게 다시 쓰게 하면 원문 인용이 바뀌고 이 제품이 주장하는 증거 가치가 무너지기 때문이다.
  원문에 그대로 없는 후보는 버린다 — `tag.ts`의 인용 검증과 같은 규율.
- **열화 정책**: LLM이 실패해도 발행을 막지 않고 정규식 결과로 진행하되, 그 사실을 view에 남겨
  검수 화면이 경고 배너를 띄운다. 사람이 무엇을 믿고 보는지 알아야 눈검사가 의미를 갖는다.
- **[계약] `PortfolioView.masking` 추가**(optional) — P2·P3 확인 필요. 옛 초안에는 이 필드가 없으므로
  화면은 "없음"과 `regex-only`를 똑같이 경고로 취급한다. 가려진 문자열은 저장하지 않고 개수만 담는다.
- `npm run portfolio -- <로그> --mask`: 데모 3종을 만들 때 발행 경로와 같은 마스킹을 적용한다.
- 마스킹 테스트 13개 추가. 전체 97개 중 95개 통과(선택 실로그 2개 제외), 린트·타입 검사·빌드 통과.

**할 일**

- 공개 포트폴리오 페이지: 타임라인 + 하이라이트 카드 + "AI 기여/인간 개입" 요약 + 사용 도구 목록 + 분석 등급 배지. OG 태그(`opengraph-image.tsx`).
- ~~마스킹 2차(LLM 검출)~~ ✅ 9/13 (아래) + 게시 전 사용자 검수 화면.
- GitHub OAuth(`@supabase/ssr`)는 후속 브랜치에서 구현·설정 완료. 내 포트폴리오 목록은 남아 있다.
- UI: shadcn/ui.
