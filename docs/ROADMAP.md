...
> 마지막 갱신: 2026-09-14 (D-6). 단계가 끝날 때마다 이 문서를 갱신한다.
...
| 7    | 포트폴리오 페이지 · 마스킹 2차 · GitHub OAuth · 검수 | 9/12~9/13 | 🔄 (공개·업로드·검수·OG와 OAuth·소유자 검사 구현, Supabase OAuth 설정 완료. 실제 계정 E2E·품질 남음) |
...
- **당시 토큰 가드** `BLOG_API_TOKEN`(fail-closed)으로 비용 라우트를 보호했다. Step 7 후속 브랜치에서 GitHub OAuth 쿠키 인증으로 교체했다.
...
**9/13 E2E 수정 — 커밋 오귀속 (`fix/p1-commit-attribution`, main 미반영)**

- 증상: 실로그 E2E 검수 화면의 커밋 배지 47개가 전부 "시간 추정"이었고, 이 세션과 무관한
  **PR #19 머지 커밋이 18번**, 커밋 하나가 finding 21번 붙었다. 포트폴리오가 남의 결과를
  내 과정으로 주장하게 된다 — 4인 팀 레포에서는 매번 발생한다.
- 2단계를 다음을 모두 만족할 때만 연결하도록 강화했다.
  ①**업로더 본인 커밋** — 프로젝트 소유자의 GitHub id(Supabase identity)와 커밋 작성자 id 대조.
  업로더를 모르면(OAuth 이전 업로드 등) 2단계를 하지 않는다.
  ②**머지 커밋 제외** — 웹 UI 머지는 버튼을 누른 사람이 작성자가 된다. #19 머지의 GitHub
  author가 실제로 업로더 본인이어서, 작성자 대조만으로는 이번 사례를 못 막았다.
  ③**작업 → 커밋 방향** — ±30분이 아니라 인용 직후 30분 안의 커밋만(시계 오차 2분 허용).
  ④**커밋당 가장 가까운 finding 2개**, ⑤1단계로 확정된 커밋에는 추정을 더하지 않음.
- 같은 로그·같은 GitHub 커밋으로 재현 비교: 수정 전 47건(E2E 화면과 일치) → 수정 후 4건
  (본인이 만든 커밋 2개에 각 2건). 테스트 12개 추가·교체.
- 남은 결함: 1단계가 실사용에서 거의 불발한다. 로그 커밋은 git 출력만 있는 청크에 떨어져
  finding이 없기 때문(실로그에서 청크 8,818자 중 사람·AI 텍스트 384자). 별도 수정 예정.
...
**9/12 화면 지원 — 역할 B (@Aio1135), PR #13 main 반영**
...
백엔드는 발행까지 준비됐고 PR #15에서 publish가 비공개 초안을 만든 뒤 검수 화면의 confirm이 공개를 확정하도록 변경됐다.
후속 인증 브랜치는 이 흐름에 로그인과 소유자 검사를 추가한다.

**9/13 인증·소유자 보호 — 역할 B, PR #18·#19 main 반영**

- Supabase GitHub OAuth 로그인·콜백·로그아웃과 Next.js 16 Proxy의 세션 갱신을 구현했다.
- 새 프로젝트에 로그인 사용자 `owner_id`를 기록하고 업로드·잡 조회/실행·초안 조립·검수·확정을 소유자에게만 허용한다.
- 공유 액세스 코드와 `BLOG_API_TOKEN` 경로를 제거하고 변경 API에 동일 출처 검사를 적용했다.
- 인증 정책·소유권 단위 테스트를 추가했다. Supabase GitHub provider와 프로덕션·로컬 Redirect URL은 9/13에 설정했고 GitHub 인증 주소까지 연결을 확인했다. 실제 계정 로그인 이후 E2E는 남아 있다.
- GitHub OAuth App은 역할 B의 `Aio1135` 계정에서 관리한다. 설정 과정에서 노출된 첫 Secret은 폐기하고 새 Secret만 Supabase에 연결했으며, 비밀값과 `.env.local`은 커밋에서 제외했다.
- 검수 보강 PR #18과 인증·소유자 보호 PR #19가 순서대로 main에 병합됐다.
- 전체 테스트 84개 중 82개 통과(선택 실로그 2개 제외), 린트·타입 검사·프로덕션 빌드 통과.

**9/14 로그인 후 작업공간 — 역할 B, `codex/post-login-dashboard`**

- 인증 사용자의 프로젝트·세션·포트폴리오를 소유권 기준으로 모아 보여주는 `/dashboard`를 추가했다.
- 검수 대기 항목은 소유자 전용 검수 화면으로, 발행 완료 항목은 공개 포트폴리오로 이어진다. 빈 계정에는 첫 분석 진입을 안내한다.
- 일반 로그인 완료 목적지를 `/dashboard`로 바꾸되 `/new`와 검수 화면에서 시작한 로그인은 원래 목적지로 복귀한다.
- 로그인 상태에 맞춰 헤더에 내 작업공간·새 로그 분석 링크를 제공한다. 실제 OAuth 승인 후 대시보드부터 발행까지의 브라우저 E2E는 남아 있다.
- 전체 테스트 85개 중 83개 통과(선택 실로그 2개 제외), 린트·타입 검사·프로덕션 빌드 통과.

**할 일**

- 공개 포트폴리오 페이지: 타임라인 + 하이라이트 카드 + "AI 기여/인간 개입" 요약 + 사용 도구 목록 + 분석 등급 배지. OG 태그(`opengraph-image.tsx`).
- 마스킹 2차(LLM 검출) + 게시 전 사용자 검수 화면.
- GitHub OAuth(`@supabase/ssr`)와 내 작업공간 목록을 구현했다. 실제 계정 E2E는 남아 있다.
- UI: shadcn/ui.
