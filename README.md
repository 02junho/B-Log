...
`/login`에서 GitHub로 로그인하면 `/dashboard`의 내 작업공간에서 분석·검수·발행 상태를 확인하고,
`/new`에서 새 로그를 분석할 수 있습니다. 로그인 만료로 `/new`나 검수 화면에서 이동한 경우에는 로그인 후 원래 화면으로 돌아갑니다.
분석 도중 창을 닫은 기록은 대시보드의 `분석 이어가기`에서 서버에 저장된 진행 지점부터 재개할 수 있습니다.
Supabase Auth의 GitHub provider와 프로덕션·로컬 Redirect URL은 9/13에 설정했습니다. 실제 계정 로그인부터 발행까지의 브라우저 E2E는 최종 확인이 남아 있습니다.
